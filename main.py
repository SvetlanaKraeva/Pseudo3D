...
length_max = max(end_length, np.amax(abs(data_events), axis=0)[3])
...
for plot_index in quasi_stages:
    ...
    for cell_index in range(number_of_full_cells):
        ...
        coord = np.array([[x_min, y_min],
                          ...
                          [x_max, y_max],
                          [x_max, y_min]])
        coord_sym = np.array([[-x_min, y_min],
                              [-x_min, y_max],
                              [-x_max, y_max],
                              [-x_max, y_min]])
        patches.append(Polygon(coord, True, linewidth=1, edgecolor='black'))
        patches.append(Polygon(coord_sym, True, linewidth=1, edgecolor='black'))
    ...
    coord_sym = np.array([[-x_min, y_min],
                          ...
                          [-x_max, y_max],
                          [-x_max, y_min]])
    patches.append(Polygon(coord, True, linewidth=2, edgecolor='black'))
    patches.append(Polygon(coord_sym, True, linewidth=2, edgecolor='black'))
    p = PatchCollection(patches)
    ax.add_collection(p)
    ax.set_xlim((-length_max, length_max))
